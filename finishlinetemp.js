...
class Marker
{
  ...
  simpleMove(diceRollValue) {
    if(diceRollValue <= deck[0].value && this.position == -1 || diceRollValue <= deck[this.next_move].value) {
      ...
      let paces = 0;
      while(paces < diceRollValue && this.card.value < diceRollValue) {
        this.next_move += 1;
        this.position += 1;
        this.card = deck[this.position];
        paces++;
      ...
      // if(deck[this.next_move].value >= diceRollValue) {
      //   this.card = deck[this.next_move];
      //   this.stop_flag = true;
      //   this.next_move += 1;
      //   this.position += 1;
      //   //break;
...
function playGame() {
  ...
  let userMarker1 = readlineSync.question(`Your black dice roll is ${player.blackDiceRoll}.
  Please provide which marker you want apply this value to `);

  player1Markers[userMarker1-1].simpleMove(player.blackDiceRoll);

  let userMarker2 = readlineSync.question(`Your red dice roll is ${player.redDiceRoll}.
  Please provide which marker you want apply this value to `);
  ...
  if(player1Markers[userMarker2-1].stop_flag) {
    ...
    player1Markers[chooseAnotherMarker-1].simpleMove(player.blackDiceRoll);
  }
  else  {
      player1Markers[userMarker2-1].simpleMove(player.redDiceRoll);
  }
  ...
  /*while(player1Markers[0].next_move < 14 && player1Markers[1].next_move < 14 && player1Markers[2].next_move < 14) {
    ...
    player1Markers[userMarker1-1].simpleMove(player1.blackDiceRoll);
    ...
    if(player1Markers[userMarker2-1].stop_flag) {
      var chooseAnotherMarker = readlineSync.question(`This marker was stopped by the stop value in the first dice roll. It cannot be chosen again.
        Please choose other marker `);
      player1Markers[chooseAnotherMarker-1].simpleMove(player1.blackDiceRoll);
    }
    else  {
        player1Markers[userMarker2-1].simpleMove(player1.redDiceRoll);
    }
